...
async function main() {
  ...
  async function deployContract(contractName, constructorArgs = [], dependencies = {}) {
    ...
    const gasEstimate = await ContractFactory.signer.estimateGas(
      ContractFactory.interface.encodeFunctionData("constructor", constructorArgs)
    );
  ...
  try {
    ...
    try {
      ...
      console.warn("⚠️  Some configuration steps failed:", error.message);
    }
